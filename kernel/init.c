...
void _start(void)
{
    ...
    /* The backgroud color must be the same as layer's col_inv */
    ...
    /* backgroud fb buffer should always be 0 */
    ...
    for (;;) {
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (256 <= i && i <= 511) { /* Keyboard event */
                ...
                if (i < 0x54 + 256) { /* Normal key */
                    if (keytable[i - 256] != 0 && cursor_x < 144) { /* Could show and have space to show */
                        s[0] = keytable[i - 256];
                        s[1] = 0;
